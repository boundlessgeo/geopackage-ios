...
- (NSArray *)databases {
  ...
  GPKGGeoPackageMetadataDao *geoPackageMetadataDao =
      [self.metadataDb getGeoPackageMetadataDao];
...
- (NSString *)pathForDatabase:(NSString *)database {
  ...
  GPKGGeoPackageMetadataDao *geoPackageMetadataDao =
      [self.metadataDb getGeoPackageMetadataDao];
  GPKGGeoPackageMetadata *metadata =
      [geoPackageMetadataDao getMetadataByName:database];
...
- (int)size:(NSString *)database {
  ...
  NSString *documentsFilename =
      [self requiredDocumentsPathForDatabase:database];
  NSError *error = nil;
  NSDictionary *fileAttributes =
      [[NSFileManager defaultManager] attributesOfItemAtPath:documentsFilename
                                                       error:&error];
  if (error) {
    [NSException
         raise:@"Database Size"
        format:@"Failed to get the size of database '%@' with error: %@",
               database, error];
  }
...
- (BOOL) delete:(NSString *)database {
  ...
  if (metadata != nil) {
    ...
    NSString *documentsPath =
        [GPKGIOUtils documentsDirectoryWithSubDirectory:metadata.path];
...
- (BOOL)deleteAll {
  ...
  if ([self count] == 0) {
    ...
    // Delete the GeoPackage database directory in case any orphan database
    // files are left
    NSError *error = nil;
    NSString *documentsDatabaseDirectory = [GPKGIOUtils
        documentsDirectoryWithSubDirectory:[GPKGIOUtils databaseDirectory]];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL directoryDeleted =
        [fileManager removeItemAtPath:documentsDatabaseDirectory error:&error];
    if (error || !directoryDeleted) {
      NSLog(@"Failed to delete GeoPackage db directory at path '%@' with "
            @"error: %@",
            documentsDatabaseDirectory, error);
    }
  }
...
- (BOOL)create:(NSString *)database inDirectory:(NSString *)dbDirectory {
  ...
  if ([self exists:database]) {
    [NSException raise:@"Database Exists"
                format:@"GeoPackage already exists: %@", database];
  }

  NSString *databasePath =
      [self buildDatabasePathWithDbDirectory:dbDirectory andDatabase:database];
  NSString *documentsDatabasePath =
      [GPKGIOUtils documentsDirectoryWithSubDirectory:databasePath];

  GPKGConnection *connection =
      [[GPKGConnection alloc] initWithDatabaseFilename:documentsDatabasePath];
  @try {
    ...
    GPKGGeoPackageTableCreator *tableCreator =
        [[GPKGGeoPackageTableCreator alloc] initWithDatabase:connection];
...
- (BOOL)importGeoPackageFromPath:(NSString *)path {
  return [self importGeoPackageFromPath:path
                               withName:nil
                            inDirectory:nil
                            andOverride:false];
}
...
- (BOOL)importGeoPackageFromPath:(NSString *)path
  ...
  if (exists) {
    if (override) {
      ...
      [NSException raise:@"Database Exists"
                  format:@"GeoPackage already exists: %@", name];
    }
  }
  ...
  if ([fileManager isReadableFileAtPath:path]) {
    NSString *databasePath =
        [self buildDatabasePathWithDbDirectory:dbDirectory
                                   andDatabase:database
                                  andExtension:[path pathExtension]];
    NSString *documentsDatabasePath =
        [GPKGIOUtils documentsDirectoryWithSubDirectory:databasePath];
    NSError *error = nil;
    if ([fileManager isReadableFileAtPath:documentsDatabasePath]) {
      [NSException
           raise:@"Import GeoPackage File"
          format:@"Failed to import GeoPackage '%@' at path '%@' to path '%@' "
                 @"because a file already exists with error: %@",
                 database, path, documentsDatabasePath, error];
    }
    if (moveFile) {
      imported = [fileManager moveItemAtPath:path
                                      toPath:documentsDatabasePath
                                       error:&error];
    } else {
      imported = [fileManager copyItemAtPath:path
                                      toPath:documentsDatabasePath
                                       error:&error];
    }
    if (error || !imported) {
      [NSException
           raise:@"Import GeoPackage File"
          format:
              @"Failed to import GeoPackage '%@' at path '%@' with error: %@",
              database, path, error];
    }

    // Validate the imported GeoPackage and create the metadata
    [self validateAndCreateImportGeoPackageWithName:database
                                            andPath:databasePath
                                   andDocumentsPath:documentsDatabasePath];
  }
...
- (void)importGeoPackageFromUrl:(NSURL *)url {
  NSString *name =
      [[[url absoluteString] lastPathComponent] stringByDeletingPathExtension];
  [self importGeoPackageFromUrl:url
                       withName:name
                    inDirectory:nil
                    andOverride:false
                    andProgress:nil];
...
- (void)importGeoPackageFromUrl:(NSURL *)url
  ...
  NSString *databasePath =
      [self buildDatabasePathWithDbDirectory:dbDirectory andDatabase:name];
  NSString *documentsDatabasePath =
      [GPKGIOUtils documentsDirectoryWithSubDirectory:databasePath];
  NSOutputStream *outputStream =
      [NSOutputStream outputStreamToFileAtPath:documentsDatabasePath
                                        append:false];
  ...
  [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation,
    ...
    if (progress == nil || [progress isActive]) {
      ...
      @try {
        // Validate the imported GeoPackage and create the metadata
        [self validateAndCreateImportGeoPackageWithName:name
                                                andPath:databasePath
                                       andDocumentsPath:documentsDatabasePath];
        ...
        NSLog(@"Download Validation Error for '%@' at url '%@' with error: %@",
              name, url, [e description]);
        if (progress != nil) {
    ...
    NSLog(@"Download Error for '%@' at url '%@' with error: %@", name, url,
          [error description]);
  }];
...
- (void)validateAndCreateImportGeoPackageWithName:(NSString *)name
  ...
  int openDatabaseResult =
      sqlite3_open([documentsPath UTF8String], &sqlite3Database);
  sqlite3_close(sqlite3Database);
  if (openDatabaseResult != SQLITE_OK) {
    // Delete the file
    [GPKGIOUtils deleteFile:documentsPath];
    [NSException raise:@"Invalid GeoPackage database file: %@"
                format:@"Invalid GeoPackage database file: %@, Error: %s",
                       documentsPath, sqlite3_errmsg(sqlite3Database)];
  }
  ...
  if (geoPackage != nil) {
      ...
      if (![[geoPackage getSpatialReferenceSystemDao] tableExists] ||
        ...
        [NSException raise:@"Invalid GeoPackage"
                    format:@"Invalid GeoPackage database file. Does not "
                           ...
                           GPKG_SRS_TABLE_NAME, GPKG_CON_TABLE_NAME, name];
      }
    } @catch (NSException *e) {
      [self delete:name];
      @throw e;
    ...
    [NSException
        ...
        format:@"Unable to open GeoPackage database. Database: %@", name];
  }
}

- (void)exportGeoPackage:(NSString *)database
...
- (GPKGGeoPackage *)open:(NSString *)database {
  ...
  if ([self exists:database]) {
    ...
    if (![fileManager fileExistsAtPath:documentsPath]) {
      [NSException raise:@"No Database File"
                  format:@"Database '%@' does not exist at path: %@", database,
                         documentsPath];
    }
    if (![fileManager isReadableFileAtPath:documentsPath]) {
      [NSException raise:@"Database Not Readable"
                  format:@"Database '%@' is not readable at path: %@", database,
                         documentsPath];
    }
    BOOL writable = [fileManager isWritableFileAtPath:documentsPath];

    GPKGConnection *db =
        [[GPKGConnection alloc] initWithDatabaseFilename:documentsPath];
    geoPackage =
        [[GPKGGeoPackage alloc] initWithConnection:db andWritable:writable];
  }
...
- (BOOL)copy:(NSString *)database
  ...
  if (sameDirectory) {
    copyPath =
        [self buildNewPathWithPath:metadata.path andNewName:databaseCopy];
  } else {
    copyPath =
        [self buildDatabasePathWithDatabase:databaseCopy
                               andExtension:[metadata.path pathExtension]];
  }
  NSString *documentsCopyPath =
      [GPKGIOUtils documentsDirectoryWithSubDirectory:copyPath];

  NSFileManager *fileManager = [NSFileManager defaultManager];
  NSError *error = nil;
  NSString *documentsPath =
      [GPKGIOUtils documentsDirectoryWithSubDirectory:metadata.path];
  BOOL copied = [fileManager copyItemAtPath:documentsPath
                                     toPath:documentsCopyPath
                                      error:&error];
  if (error || !copied) {
    [NSException
         raise:@"Copy GeoPackage"
        format:@"Failed to copy GeoPackage '%@' at path '%@' to GeoPackage "
               @"'%@' at path '%@' with error: %@",
               database, documentsPath, databaseCopy, documentsCopyPath, error];
  }

  if (copied) {
    [self createMetadataWithDao:geoPackageMetadataDao
                        andName:databaseCopy
                        andPath:copyPath];
    copied = true;
  }
...
- (BOOL)rename:(NSString *)database
 ...
 andRenameFile:(BOOL)renameFile {
  ...
  if (renameFile) {
    ...
    NSString *documentsRenamePath =
        [GPKGIOUtils documentsDirectoryWithSubDirectory:renamePath];

    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    NSString *documentsPath =
        [GPKGIOUtils documentsDirectoryWithSubDirectory:metadata.path];
    success = [fileManager moveItemAtPath:documentsPath
                                   toPath:documentsRenamePath
                                    error:&error];
    if (error || !success) {
      [NSException raise:@"Rename GeoPackage"
                  format:@"Failed to rename GeoPackage '%@' at path '%@' to "
                         @"GeoPackage '%@' at path '%@' with error: %@",
                         database, documentsPath, newDatabase,
                         documentsRenamePath, error];
    }
  }
...
- (BOOL)move:(NSString *)database toDirectory:(NSString *)dbDirectory {
  ...
  NSString *movePath = [dbDirectory
      stringByAppendingPathComponent:[metadata.path lastPathComponent]];
  NSString *documentsMovePath =
      [GPKGIOUtils documentsDirectoryWithSubDirectory:movePath];
  ...
  NSString *documentsPath =
      [GPKGIOUtils documentsDirectoryWithSubDirectory:metadata.path];
  BOOL success =
      [fileManager moveItemAtPath:documentsPath toPath:movePath error:&error];
  if (error || !success) {
    [NSException raise:@"Move GeoPackage"
                format:@"Failed to move GeoPackage '%@' at path '%@' to path "
                       @"'%@' with error: %@",
                       database, documentsPath, documentsMovePath, error];
  }
...
- (void)createMetadataWithDao:(GPKGGeoPackageMetadataDao *)dao
...
- (NSString *)buildDatabasePathWithDatabase:(NSString *)database {
  return [self buildDatabasePathWithDatabase:database
                                andExtension:GPKG_GEOPACKAGE_EXTENSION];
}

- (NSString *)buildDatabasePathWithDatabase:(NSString *)database
                               andExtension:(NSString *)extension {
  return [self buildDatabasePathWithDbDirectory:nil
                                    andDatabase:database
                                   andExtension:extension];
}

- (NSString *)buildDatabasePathWithDbDirectory:(NSString *)dbDirectory
                                   andDatabase:(NSString *)database {
  return [self buildDatabasePathWithDbDirectory:dbDirectory
                                    andDatabase:database
                                   andExtension:GPKG_GEOPACKAGE_EXTENSION];
}
...
- (NSString *)buildDatabasePathWithDbDirectory:(NSString *)dbDirectory
  ...
  NSString *databasePath = [dbDirectory
      stringByAppendingPathComponent:
          [NSString stringWithFormat:@"%@.%@", database, extension]];
  return databasePath;
}

- (NSString *)buildNewPathWithPath:(NSString *)path
