...
createTileTableWithTableName:(NSString *)tableName
 ...
 andTileMatrixSetBoundingBox:(GPKGBoundingBox *)tileMatrixSetBoundingBox
  ...
  GPKGSpatialReferenceSystem *tileMatrixSetSrs =
      [self getSrs:tileMatrixSetSrsId];
...
- (void)verifyWritable {
  if (!self.writable) {
    [NSException
         raise:@"Read Only"
        format:@"GeoPackage file is not writable. Name: %@%@", self.name,
               (self.path != nil
                    ? [NSString stringWithFormat:@", Path: %@", self.path]
                    : @"")];
  }
}
...
- (GPKGFeatureDao *)getFeatureDaoWithGeometryColumns:
  ...
  GPKGFeatureTable *featureTable =
      [tableReader readFeatureTableWithConnection:self.database];
  GPKGFeatureDao *dao =
...
- (GPKGFeatureDao *)getFeatureDaoWithTableName:(NSString *)tableName {
  ...
  if (geometryColumns == nil) {
    [NSException
        ...
        format:@"No Feature Table exists for table name: %@", tableName];
  }
  return [self getFeatureDaoWithGeometryColumns:geometryColumns];
}

- (GPKGTileDao *)getTileDaoWithTileMatrixSet:
    (GPKGTileMatrixSet *)tileMatrixSet {
  if (tileMatrixSet == nil) {
    [NSException
         raise:@"Illegal Argument"
        format:@"Non null Tile Matrix Set is required to create Tile DAO"];
  }
  ...
  GPKGResultSet *results = [tileMatrixDao
      queryForEqWithField:GPKG_TM_COLUMN_TABLE_NAME
                 andValue:tileMatrixSet.tableName
               andGroupBy:nil
                andHaving:nil
               andOrderBy:[NSString
                              stringWithFormat:@"%@ ASC, %@ DESC, %@ DESC",
                                               GPKG_TM_COLUMN_ZOOM_LEVEL,
                                               GPKG_TM_COLUMN_PIXEL_X_SIZE,
                                               GPKG_TM_COLUMN_PIXEL_Y_SIZE]];
  @try {
    while ([results moveToNext]) {
      GPKGTileMatrix *tileMatrix =
          (GPKGTileMatrix *)[tileMatrixDao getObject:results];
  ...
  // Read the existing table and create the dao
  GPKGTileTableReader *tableReader =
      [[GPKGTileTableReader alloc] initWithTableName:tileMatrixSet.tableName];
  GPKGTileTable *tileTable =
      [tableReader readTileTableWithConnection:self.database];
  GPKGTileDao *dao = [[GPKGTileDao alloc] initWithDatabase:self.database
                                                  andTable:tileTable
                                          andTileMatrixSet:tileMatrixSet
                                           andTileMatrices:tileMatrices];
  return dao;
}
...
- (GPKGTileDao *)getTileDaoWithTableName:(NSString *)tableName {
  ...
  @try {
    tileMatrixSetList =
        [dao queryForEqWithField:GPKG_TMS_COLUMN_TABLE_NAME andValue:tableName];
    if ([tileMatrixSetList moveToNext]) {
      tileMatrixSet = (GPKGTileMatrixSet *)[dao getObject:tileMatrixSetList];
      if ([tileMatrixSetList moveToNext]) {
        [NSException raise:@"Multiple Tile Matrix Sets"
                    format:@"Unexpected state. More than one Tile Matrix Set "
                           @"matched for table name: %@, count: %d",
                           tableName, [tileMatrixSetList count]];
      }
    }
  } @finally {
  ...
  if (tileMatrixSet == nil) {
    [NSException raise:@"No Tile Matrix Set"
                format:@"No Tile Table exists for table name: %@", tableName];
  }
...
- (void)dropSQLiteTriggers:(GPKGGeometryColumns *)geometryColumns {
  ...
  if (self.writable) {
    [self.database
        exec:[NSString
                 stringWithFormat:@"DROP TRIGGER IF EXISTS rtree_%@_%@_insert",
                                  geometryColumns.tableName,
                                  geometryColumns.columnName]];
    [self.database
        exec:[NSString
                 stringWithFormat:@"DROP TRIGGER IF EXISTS rtree_%@_%@_update1",
                                  geometryColumns.tableName,
                                  geometryColumns.columnName]];
    [self.database
        exec:[NSString
                 stringWithFormat:@"DROP TRIGGER IF EXISTS rtree_%@_%@_update2",
                                  geometryColumns.tableName,
                                  geometryColumns.columnName]];
    [self.database
        exec:[NSString
                 stringWithFormat:@"DROP TRIGGER IF EXISTS rtree_%@_%@_update3",
                                  geometryColumns.tableName,
                                  geometryColumns.columnName]];
    [self.database
        exec:[NSString
                 stringWithFormat:@"DROP TRIGGER IF EXISTS rtree_%@_%@_update4",
                                  geometryColumns.tableName,
                                  geometryColumns.columnName]];
    [self.database
        exec:[NSString
                 stringWithFormat:@"DROP TRIGGER IF EXISTS rtree_%@_%@_delete",
                                  geometryColumns.tableName,
                                  geometryColumns.columnName]];
  }
}
...
- (GPKGSpatialReferenceSystem *)getSrs:(NSNumber *)srsId {
  ...
  GPKGSpatialReferenceSystem *srs =
      (GPKGSpatialReferenceSystem *)[dao queryForIdObject:srsId];
  if (srs == nil) {
    [NSException
         raise:@"No SRS"
        format:@"Spatial Reference System could not be found. SRS ID: %@",
               srsId];
  }
  return srs;
}
