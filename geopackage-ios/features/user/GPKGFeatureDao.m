...
- (instancetype)initWithDatabase:(GPKGConnection *)database
  ...
  if (self != nil) {
    ...
    if ([dao getContents:geometryColumns] == nil) {
      [NSException raise:@"Missing Contents"
                  format:@"Geometry Columns %@ has null Contents",
                         [dao getId:geometryColumns]];
    }
    if ([dao getSrs:geometryColumns] == nil) {
      [NSException
           raise:@"Missing SRS"
          format:@"Geometry Columns %@ has null Spatial Reference System",
                 [dao getId:geometryColumns]];
    }
...
- (GPKGUserRow *)newRowWithColumnTypes:(NSArray *)columnTypes
  ...
  return [[GPKGFeatureRow alloc] initWithFeatureTable:[self getFeatureTable]
                                       andColumnTypes:columnTypes
                                            andValues:values];
}

- (GPKGFeatureRow *)newRow {
  return [[GPKGFeatureRow alloc]
      initWithFeatureTable:(GPKGFeatureTable *)self.table];
}
...
- (GPKGBoundingBox *)getBoundingBox {
  ...
  GPKGContents *contents =
      [geometryColumnsDao getContents:self.geometryColumns];
  ...
  if ([self.projection.epsg compare:contentsProjection.epsg] != NSOrderedSame) {
    GPKGProjectionTransform *transform = [[GPKGProjectionTransform alloc]
        initWithFromProjection:contentsProjection
               andToProjection:self.projection];
