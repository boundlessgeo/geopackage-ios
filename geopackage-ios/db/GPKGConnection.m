...
- (id)initWithDatabaseFilename:(NSString *)filename {
  ...
  if (self) {
    ...
    self.connectionPool =
        [[GPKGConnectionPool alloc] initWithDatabaseFilename:filename];
  }
...
- (int)count:(NSString *)statement andArgs:(NSArray *)args {
  ...
  int count = [GPKGSqlUtils countWithDatabase:connection
                                 andStatement:statement
                                      andArgs:args];
...
- (int)countWithTable:(NSString *)table andWhere:(NSString *)where {
  GPKGDbConnection *connection = [self.connectionPool getConnection];
  int count =
      [GPKGSqlUtils countWithDatabase:connection andTable:table andWhere:where];
  [self.connectionPool releaseConnection:connection];
  return count;
}
...
- (long long)insert:(NSString *)statement {
  ...
  long long id =
      [GPKGSqlUtils insertWithDatabase:connection andStatement:statement];
  [self.connectionPool releaseConnection:connection];
  return id;
}
...
- (int)update:(NSString *)statement {
  GPKGDbConnection *connection = [self.connectionPool getWriteConnection];
  int count =
      [GPKGSqlUtils updateWithDatabase:connection andStatement:statement];
  [self.connectionPool releaseConnection:connection];
  return count;
}

- (int)updateWithTable:(NSString *)table
             andValues:(GPKGContentValues *)values
              andWhere:(NSString *)where {
  GPKGDbConnection *connection = [self.connectionPool getWriteConnection];
  int count = [GPKGSqlUtils updateWithDatabase:connection
                                      andTable:table
                                     andValues:values
                                      andWhere:where];
  [self.connectionPool releaseConnection:connection];
...
- (long long)insertWithTable:(NSString *)table
                   andValues:(GPKGContentValues *)values {
  GPKGDbConnection *connection = [self.connectionPool getWriteConnection];
  long long id = [GPKGSqlUtils insertWithDatabase:connection
                                         andTable:table
                                        andValues:values];
  [self.connectionPool releaseConnection:connection];
  return id;
...
- (int) delete:(NSString *)statement {
  GPKGDbConnection *connection = [self.connectionPool getWriteConnection];
  int count =
      [GPKGSqlUtils deleteWithDatabase:connection andStatement:statement];
  [self.connectionPool releaseConnection:connection];
  return count;
}
...
- (int)deleteWithTable:(NSString *)table andWhere:(NSString *)where {
  GPKGDbConnection *connection = [self.connectionPool getWriteConnection];
  int count = [GPKGSqlUtils deleteWithDatabase:connection
                                      andTable:table
                                      andWhere:where];
  [self.connectionPool releaseConnection:connection];
  return count;
...
- (int)deleteWithTable:(NSString *)table
              andWhere:(NSString *)where
          andWhereArgs:(NSArray *)whereArgs {
  GPKGDbConnection *connection = [self.connectionPool getWriteConnection];
  int count = [GPKGSqlUtils deleteWithDatabase:connection
...
- (BOOL)tableExists:(NSString *)table {
  int count = [self
      countWithTable:@"sqlite_master"
            andWhere:[NSString
                         stringWithFormat:@"type ='table' and name = '%@'",
                                          table]];
  BOOL found = count > 0;
  return found;
}
...
- (void)setApplicationId:(NSString *)applicationId {
  ...
  [self exec:[NSString stringWithFormat:@"PRAGMA application_id = %d",
                                        applicationIdData]];
}
