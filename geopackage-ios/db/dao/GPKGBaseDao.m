...
- (void)setValueInObject:(NSObject *)object
         withColumnIndex:(int)columnIndex
               withValue:(NSObject *)value {
  [self doesNotRecognizeSelector:_cmd];
}

- (NSObject *)getValueFromObject:(NSObject *)object
                 withColumnIndex:(int)columnIndex {
  [self doesNotRecognizeSelector:_cmd];
  return nil;
}
...
- (void)setValueInObject:(NSObject *)object
          withColumnName:(NSString *)column
               withValue:(NSObject *)value {
  int columnIndex =
      ((NSNumber *)[self.columnIndex valueForKey:column]).intValue;
...
- (void)initializeColumnIndex {
  ...
  for (int i = 0; i < count; i++) {
    [GPKGUtils setObject:[NSNumber numberWithInt:i]
                  forKey:[GPKGUtils objectAtIndex:i inArray:self.columns]
            inDictionary:self.columnIndex];
  }
}
...
- (GPKGResultSet *)queryForId:(NSObject *)idValue {
  ...
  GPKGResultSet *results = [self.database queryWithTable:self.tableName
                                              andColumns:nil
...
- (GPKGResultSet *)queryForAll {
  return [self.database queryWithTable:self.tableName
                            andColumns:nil
                              andWhere:nil
                          andWhereArgs:nil
                            andGroupBy:nil
...
- (NSObject *)getObject:(GPKGResultSet *)results {
  ...
  for (int i = 0; i < count; i++) {
    [self setValueInObject:objectResult
            withColumnName:[GPKGUtils objectAtIndex:i inArray:results.columns]
                 withValue:[GPKGUtils objectAtIndex:i inArray:result]];
  }
...
- (NSArray *)singleColumnResults:(GPKGResultSet *)results {
  ...
  while ([results moveToNext]) {
    NSArray *result = [results getRow];
    [GPKGUtils addObject:[GPKGUtils objectAtIndex:0 inArray:result]
                 toArray:singleColumnResults];
  }
...
- (GPKGResultSet *)queryForEqWithField:(NSString *)field
  ...
  return [self queryForEqWithField:field
                          andValue:value
                        andGroupBy:nil
                         andHaving:nil
                        andOrderBy:nil];
...
- (int)update:(NSObject *)object {
  ...
  int count =
      [self updateWithValues:values andWhere:where andWhereArgs:whereArgs];
  return count;
}
...
- (int)updateWithValues:(GPKGContentValues *)values
               andWhere:(NSString *)where
           andWhereArgs:(NSArray *)whereArgs {
  int count = [self.database updateWithTable:self.tableName
                                   andValues:values
                                    andWhere:where
                                andWhereArgs:whereArgs];
  return count;
...
- (int)deleteById:(NSObject *)idValue {
  return
      [self.database deleteWithTable:self.tableName
                            andWhere:[self buildPkWhereWithValue:idValue]
                        andWhereArgs:[self buildPkWhereArgsWithValue:idValue]];
}

- (int)deleteByMultiId:(NSArray *)idValues {
  return [self.database
      deleteWithTable:self.tableName
             andWhere:[self buildPkWhereWithValues:idValues]
         andWhereArgs:[self buildPkWhereArgsWithValues:idValues]];
}

- (int)deleteWhere:(NSString *)where andWhereArgs:(NSArray *)whereArgs {
  return [self.database deleteWithTable:self.tableName
                               andWhere:where
                           andWhereArgs:whereArgs];
}
...
- (long long)insert:(NSObject *)object {
  ...
  long long id =
      [self.database insertWithTable:self.tableName andValues:values];
  return id;
}
...
- (NSObject *)getId:(NSObject *)object {
  return [self
      getValueFromObject:object
          withColumnName:[GPKGUtils objectAtIndex:0 inArray:self.idColumns]];
}
...
- (NSArray *)getMultiId:(NSObject *)object {
  ...
  for (NSString *idColumn in self.idColumns) {
    NSObject *idValue =
        [self getValueFromObject:object withColumnName:idColumn];
...
- (void)setMultiId:(NSObject *)object withIdValues:(NSArray *)idValues {
  for (int i = 0; i < [idValues count]; i++) {
    [self setValueInObject:object
            withColumnName:[GPKGUtils objectAtIndex:i inArray:self.idColumns]
                 withValue:[GPKGUtils objectAtIndex:i inArray:idValues]];
  }
}

- (NSString *)buildPkWhereWithValue:(NSObject *)idValue {
  return [self
      buildWhereWithField:[GPKGUtils objectAtIndex:0 inArray:self.idColumns]
                 andValue:idValue];
}
...
- (NSString *)buildPkWhereWithValues:(NSArray *)idValues {
  ...
  for (int i = 0; i < [idValues count]; i++) {
    [idColumnValues addColumn:[GPKGUtils objectAtIndex:i inArray:self.idColumns]
                    withValue:[GPKGUtils objectAtIndex:i inArray:idValues]];
  }
...
- (NSString *)buildWhereWithFields:(GPKGColumnValues *)fields {
  ...
  for (NSString *column in fields.columns) {
    ...
    [whereString
        appendString:[self buildWhereWithField:column
                                      andValue:[fields getValue:column]]];
  }
  return whereString;
}
...
- (NSString *)buildWhereWithColumnValueFields:(GPKGColumnValues *)fields {
  ...
  for (NSString *column in fields.columns) {
    ...
    [whereString
        appendString:[self buildWhereWithField:column
                                andColumnValue:(GPKGColumnValue *)[fields
                                                   getValue:column]]];
  }
  return whereString;
}
...
- (NSString *)buildWhereWithField:(NSString *)field
  ...
  if (value != nil) {
    if (value.value != nil && value.tolerance != nil) {
      ...
      [whereString
          appendString:[self buildWhereWithField:field andValue:value.value]];
    }
  } else {
...
- (int)countWhere:(NSString *)where andWhereArgs:(NSArray *)args {
  return [self.database countWithTable:self.tableName
                              andWhere:where
                          andWhereArgs:args];
}
...
- (NSArray *)getValueToleranceRange:(GPKGColumnValue *)value {
  ...
  return [NSArray arrayWithObjects:[[NSDecimalNumber alloc]
                                       initWithDouble:doubleValue - tolerance],
