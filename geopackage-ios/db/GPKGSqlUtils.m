...
+ (void)execWithDatabase:(GPKGDbConnection *)connection
            andStatement:(NSString *)statement {

  char *errInfo;
  int result = sqlite3_exec([connection getConnection], [statement UTF8String],
                            nil, nil, &errInfo);

  if (SQLITE_OK != result) {
    NSString *err = [[NSString alloc] initWithUTF8String:errInfo];
    [NSException raise:@"SQL Failed"
                format:@"Failed to execute SQL: %@, Error: %@", statement, err];
  }
}
...
+ (GPKGResultSet *)queryWithDatabase:(GPKGDbConnection *)connection
  ...
  int count =
      [self countWithDatabase:connection andStatement:statement andArgs:args];

  sqlite3_stmt *compiledStatement;
  int prepareStatementResult =
      sqlite3_prepare_v2([connection getConnection], [statement UTF8String], -1,
  ...
  if (prepareStatementResult == SQLITE_OK) {
    ...
    [NSException raise:@"SQL Failed"
                format:@"Failed to execute query SQL: %@, Error: %s", statement,
                       sqlite3_errmsg([connection getConnection])];
  }
...
+ (GPKGResultSet *)queryWithDatabase:(GPKGDbConnection *)connection
  ...
  GPKGResultSet *resultSet =
      [self queryWithDatabase:connection andStatement:query andArgs:whereArgs];
  return resultSet;
}

+ (int)countWithDatabase:(GPKGDbConnection *)connection
            andStatement:(NSString *)statement
                 andArgs:(NSArray *)args {
  ...
  if (![countStatement containsString:@" count(*) "]) {
    ...
    countStatement =
        [NSString stringWithFormat:@"select count(*)%@",
                                   [countStatement substringFromIndex:index]];
  }

  int count = [self countWithDatabase:connection
                    andCountStatement:countStatement
                              andArgs:args];
...
+ (int)countWithDatabase:(GPKGDbConnection *)connection
                andTable:(NSString *)table
                andWhere:(NSString *)where {
  return [self countWithDatabase:connection
                        andTable:table
                        andWhere:where
                    andWhereArgs:nil];
}
...
+ (int)countWithDatabase:(GPKGDbConnection *)connection
       andCountStatement:(NSString *)countStatement {
  return [self countWithDatabase:connection
               andCountStatement:countStatement
                         andArgs:nil];
}
...
+ (int)countWithDatabase:(GPKGDbConnection *)connection
  ...
  sqlite3_stmt *compiledStatement;
  int prepareStatementResult = sqlite3_prepare_v2([connection getConnection],
                                                  [countStatement UTF8String],
  ...
  if (prepareStatementResult == SQLITE_OK) {
    ...
    if (sqlite3_step(compiledStatement) == SQLITE_ROW) {
      ...
      [NSException raise:@"SQL Failed"
                  format:@"Failed to get count: %@, Error: %s", countStatement,
                         sqlite3_errmsg([connection getConnection])];
    }
  } else {
    [NSException
         raise:@"SQL Failed"
        format:@"Failed to execute count SQL: %@, Error: %s", countStatement,
               sqlite3_errmsg([connection getConnection])];
  }
...
+ (long long)insertWithDatabase:(GPKGDbConnection *)connection
                   andStatement:(NSString *)statement {
  return
      [self insertWithDatabase:connection andStatement:statement andArgs:nil];
}

+ (long long)insertWithDatabase:(GPKGDbConnection *)connection
                       andTable:(NSString *)table
                      andValues:(GPKGContentValues *)values {
  ...
  long long id = [self insertWithDatabase:connection
                             andStatement:insertStatement
                                  andArgs:args];
...
+ (long long)insertWithDatabase:(GPKGDbConnection *)connection
  ...
  if (prepareStatementResult == SQLITE_OK) {
    ...
    if (executeQueryResults == SQLITE_DONE) {
      ...
      [NSException raise:@"SQL Failed"
                  format:@"Failed to execute insert SQL: %@, Error: %s",
                         statement, sqlite3_errmsg([connection getConnection])];
    }
  } else {
    [NSException raise:@"SQL Failed"
                format:@"Failed to execute insert SQL: %@, Error: %s",
...
+ (int)updateWithDatabase:(GPKGDbConnection *)connection
             andStatement:(NSString *)statement {
  return
      [self updateWithDatabase:connection andStatement:statement andArgs:nil];
}

+ (int)updateWithDatabase:(GPKGDbConnection *)connection
             andStatement:(NSString *)statement
                  andArgs:(NSArray *)args {
  return [self updateOrDeleteWithDatabase:connection
...
+ (int)updateWithDatabase:(GPKGDbConnection *)connection
                ...
                andValues:(GPKGContentValues *)values
                 andWhere:(NSString *)where {
  return [self updateWithDatabase:connection
                         andTable:table
                        andValues:values
                         andWhere:where
                     andWhereArgs:nil];
...
+ (int)updateWithDatabase:(GPKGDbConnection *)connection
  ...
  int argsSize = (whereArgs == nil) ? setValuesSize
                                    : (setValuesSize + (int)[whereArgs count]);
  ...
  int count = [self updateWithDatabase:connection
                          andStatement:updateStatement
                               andArgs:args];

  return count;
}

+ (int)deleteWithDatabase:(GPKGDbConnection *)connection
             andStatement:(NSString *)statement {
  return
      [self deleteWithDatabase:connection andStatement:statement andArgs:nil];
}
...
+ (int)deleteWithDatabase:(GPKGDbConnection *)connection
                 andTable:(NSString *)table
                 andWhere:(NSString *)where {
  return [self deleteWithDatabase:connection
                         andTable:table
                         andWhere:where
                     andWhereArgs:nil];
}
...
+ (int)deleteWithDatabase:(GPKGDbConnection *)connection
  ...
  int count = [self deleteWithDatabase:connection
                          andStatement:deleteStatement
                               andArgs:whereArgs];

  return count;
}

+ (int)updateOrDeleteWithDatabase:(GPKGDbConnection *)connection
                     andStatement:(NSString *)statement
                          andArgs:(NSArray *)args {
  ...
  if (prepareStatementResult == SQLITE_OK) {
    ...
    if (executeQueryResults == SQLITE_DONE) {
      ...
      [NSException
           raise:@"SQL Failed"
          format:@"Failed to execute update or delete SQL: %@, Error: %s",
                 statement, sqlite3_errmsg([connection getConnection])];
    }
  } else {
    [NSException raise:@"SQL Failed"
  ...
  if (arguments != nil) {
    for (int i = 0; i < [arguments count]; i++) {
      ...
      if ([argument isKindOfClass:[NSData class]]) {
        ...
        int bindResult =
            sqlite3_bind_blob(statement, index, [data bytes],
                              (int)[data length], SQLITE_TRANSIENT);
        if (bindResult != SQLITE_OK) {
          [NSException
               raise:@"Bind Blob"
              format:
                  @"Failed to bind blob in SQL statement: %@, Error Code: %d",
        ...
        int bindResult = sqlite3_bind_text(
            statement, index, [dateString UTF8String], -1, SQLITE_TRANSIENT);
        if (bindResult != SQLITE_OK) {
          [NSException
               raise:@"Bind Date"
              format:
                  @"Failed to bind date in SQL statement: %@, Error Code: %d",
                  statement, bindResult];
        }
      } else if ([argument isKindOfClass:[NSString class]]) {
        NSString *string = (NSString *)argument;
        int bindResult = sqlite3_bind_text(
            statement, index, [string UTF8String], -1, SQLITE_TRANSIENT);
        if (bindResult != SQLITE_OK) {
          [NSException
               raise:@"Bind String"
              format:
                  @"Failed to bind string in SQL statement: %@, Error Code: %d",
                  statement, bindResult];
        }
        ...
        int bindResult = sqlite3_bind_text(
            statement, index, [decimalString UTF8String], -1, SQLITE_TRANSIENT);
        if (bindResult != SQLITE_OK) {
          [NSException raise:@"Bind Decimal"
                      format:@"Failed to bind decimal in SQL statement: %@, "
                             @"Error Code: %d",
                             statement, bindResult];
        }
        ...
        if (numberType == kCFNumberFloat32Type ||
          ...
          double value;
          success = CFNumberGetValue((CFNumberRef)number, kCFNumberFloat64Type,
                                     &value);
          if (success) {
            success =
          ...
          SInt64 value;
          success = CFNumberGetValue((CFNumberRef)number, kCFNumberSInt64Type,
                                     &value);
          if (success) {
            success =
        ...
        if (!success) {
          ...
          int bindResult =
              sqlite3_bind_text(statement, index, [numberString UTF8String], -1,
                                SQLITE_TRANSIENT);
          if (bindResult != SQLITE_OK) {
            [NSException raise:@"Bind Number"
                        format:@"Failed to bind number in SQL statement: %@, "
                               @"Error Code: %d",
                               statement, bindResult];
          }
        }
        ...
        if (bindResult != SQLITE_OK) {
          ...
          //                    [NSException raise:@"Bind Null" format:@"Failed
          //                    to bind null in SQL statement: %@, Error Code:
          //                    %d", statement, bindResult];
        }
      } else {
        [NSException raise:@"Unsupported Bind"
                    format:@"Unsupported bind type: %@",
                           NSStringFromClass([argument class])];
      }
    }
  }
}
