...
+ (void)assertEqualWithValue:(NSObject *)value andValue2:(NSObject *)value2 {
  if (value == nil) {
    if (value2 != nil) {
      [NSException
           raise:@"Assert Equal"
          format:@"Value 1: '%@' is not equal to Value 2: '%@'", value, value2];
    }
...
+ (void)assertEqualBoolWithValue:(BOOL)value andValue2:(BOOL)value2 {
  if (value != value2) {
    [NSException
         raise:@"Assert Equal BOOL"
        format:@"Value 1: '%d' is not equal to Value 2: '%d'", value, value2];
  }
}
...
+ (void)assertEqualDoubleWithValue:(double)value andValue2:(double)value2 {
  if (value != value2) {
    [NSException
         raise:@"Assert Equal double"
        format:@"Value 1: '%f' is not equal to Value 2: '%f'", value, value2];
  }
}
...
+ (void)createConstraints:(GPKGGeoPackage *)geoPackage {
  ...
  GPKGDataColumnConstraints *sampleRange =
      [[GPKGDataColumnConstraints alloc] init];
  ...
  GPKGDataColumnConstraints *sampleEnum1 =
      [[GPKGDataColumnConstraints alloc] init];
  ...
  GPKGDataColumnConstraints *sampleEnum3 =
      [[GPKGDataColumnConstraints alloc] init];
  ...
  GPKGDataColumnConstraints *sampleEnum5 =
      [[GPKGDataColumnConstraints alloc] init];
  ...
  GPKGDataColumnConstraints *sampleEnum7 =
      [[GPKGDataColumnConstraints alloc] init];
  ...
  GPKGDataColumnConstraints *sampleEnum9 =
      [[GPKGDataColumnConstraints alloc] init];
  ...
  GPKGDataColumnConstraints *sampleGlob =
      [[GPKGDataColumnConstraints alloc] init];
...
+ (GPKGFeatureTable *)
createFeatureTableWithGeoPackage:(GPKGGeoPackage *)geoPackage
                     andContents:(GPKGContents *)contents
               andGeometryColumn:(NSString *)geometryColumn
                 andGeometryType:(enum WKBGeometryType)geometryType {

  GPKGFeatureTable *table =
      [self buildFeatureTableWithTableName:contents.tableName
                         andGeometryColumn:geometryColumn
  ...
  case 0:
    [dataColumns
        setConstraintName:GPKG_GEOPACKAGE_TEST_SAMPLE_RANGE_CONSTRAINT];
    break;
  case 1:
...
+ (GPKGFeatureTable *)buildFeatureTableWithTableName:(NSString *)tableName
                                   andGeometryColumn:(NSString *)geometryColumn
                                     andGeometryType:
  ...
  [GPKGUtils addObject:[GPKGFeatureColumn createPrimaryKeyColumnWithIndex:0
                                                                  andName:@"id"]
               toArray:columns];
  [GPKGUtils addObject:[GPKGFeatureColumn
                           createColumnWithIndex:7
                                         andName:@"test_text_limited"
                                     andDataType:GPKG_DT_TEXT
                                          andMax:[NSNumber numberWithInt:5]
                                      andNotNull:false
                                 andDefaultValue:nil]
               toArray:columns];
  [GPKGUtils addObject:[GPKGFeatureColumn
                           createColumnWithIndex:8
                                         andName:@"test_blob_limited"
                                     andDataType:GPKG_DT_BLOB
                                          andMax:[NSNumber numberWithInt:7]
                                      andNotNull:false
                                 andDefaultValue:nil]
               toArray:columns];
  [GPKGUtils
      addObject:[GPKGFeatureColumn createGeometryColumnWithIndex:1
                                                         andName:geometryColumn
                                                 andGeometryType:geometryType
                                                      andNotNull:false
                                                 andDefaultValue:nil]
        toArray:columns];
  [GPKGUtils addObject:[GPKGFeatureColumn createColumnWithIndex:2
                                                        andName:@"test_text"
                                                    andDataType:GPKG_DT_TEXT
                                                     andNotNull:false
                                                andDefaultValue:@""]
               toArray:columns];
  [GPKGUtils addObject:[GPKGFeatureColumn createColumnWithIndex:3
                                                        andName:@"test_real"
                                                    andDataType:GPKG_DT_REAL
                                                     andNotNull:false
                                                andDefaultValue:nil]
               toArray:columns];
  [GPKGUtils addObject:[GPKGFeatureColumn createColumnWithIndex:4
                                                        andName:@"test_boolean"
                                                    andDataType:GPKG_DT_BOOLEAN
                                                     andNotNull:false
                                                andDefaultValue:nil]
               toArray:columns];
  [GPKGUtils addObject:[GPKGFeatureColumn createColumnWithIndex:5
                                                        andName:@"test_blob"
                                                    andDataType:GPKG_DT_BLOB
                                                     andNotNull:false
                                                andDefaultValue:nil]
  ...
  [GPKGUtils
      addObject:[GPKGFeatureColumn
                    createColumnWithIndex:6
                                  andName:GPKG_GEOPACKAGE_TEST_INTEGER_COLUMN
                              andDataType:GPKG_DT_INTEGER
                               andNotNull:false
                          andDefaultValue:nil]
        toArray:columns];

  GPKGFeatureTable *table =
      [[GPKGFeatureTable alloc] initWithTable:tableName andColumns:columns];
...
+ (GPKGTileTable *)buildTileTableWithTableName:(NSString *)tableName {
  ...
  GPKGTileTable *table =
      [[GPKGTileTable alloc] initWithTable:tableName andColumns:columns];

  return table;
}

+ (void)addRowsToFeatureTableWithGeoPackage:(GPKGGeoPackage *)geoPackage
                         andGeometryColumns:
                             (GPKGGeometryColumns *)geometryColumns
                            andFeatureTable:(GPKGFeatureTable *)table
                                 andNumRows:(int)numRows
                                    andHasZ:(BOOL)hasZ
                                    andHasM:(BOOL)hasM {

  GPKGFeatureDao *dao =
      [geoPackage getFeatureDaoWithGeometryColumns:geometryColumns];
    ...
    for (GPKGFeatureColumn *column in table.columns) {
      if (!column.primaryKey) {
        ...
        if ([column isGeometry]) {
          ...
          case WKB_LINESTRING:
            geometry =
                [self createLineStringWithHasZ:hasZ andHasM:hasM andRing:false];
            break;
          case WKB_POLYGON:
          ...
          default:
            [NSException raise:@"Not implemented"
                        format:@"Not implemented for geometry type: %u",
                               column.geometryType];
          }

          GPKGGeometryData *geometryData =
              [[GPKGGeometryData alloc] initWithSrsId:geometryColumns.srsId];
          ...
          case GPKG_DT_BOOLEAN:
            value = [NSNumber
                numberWithBool:([self randomDouble] < .5 ? false : true)];
            break;
          ...
          case GPKG_DT_INT:
            ...
            break;
          case GPKG_DT_BLOB: {
            NSData *blob = [[[NSProcessInfo processInfo] globallyUniqueString]
                dataUsingEncoding:NSUTF8StringEncoding];
            if (column.max != nil && [blob length] > [column.max intValue]) {
              blob =
                  [blob subdataWithRange:NSMakeRange(0, [column.max intValue])];
            }
            value = blob;
          } break;
          default:
            [NSException
                ...
                format:@"Not implemented for data type: %u", column.dataType];
          }
...
+ (void)addRowsToTileTableWithGeoPackage:(GPKGGeoPackage *)geoPackage
                           andTileMatrix:(GPKGTileMatrix *)tileMatrix
                                 andData:(NSData *)tileData {
...
+ (WKBPoint *)createPointWithHasZ:(BOOL)hasZ andHasM:(BOOL)hasM {
  ...
  double x =
      [self randomDoubleLessThan:180.0] * ([self randomDouble] < .5 ? 1 : -1);
  double y =
      [self randomDoubleLessThan:90.0] * ([self randomDouble] < .5 ? 1 : -1);

  NSDecimalNumber *xNumber = [self roundDouble:x];
  NSDecimalNumber *yNumber = [self roundDouble:y];

  WKBPoint *point = [[WKBPoint alloc] initWithHasZ:hasZ
                                           andHasM:hasM
                                              andX:xNumber
                                              andY:yNumber];
...
+ (WKBLineString *)createLineStringWithHasZ:(BOOL)hasZ
                                    andHasM:(BOOL)hasM
                                    andRing:(BOOL)ring {

  WKBLineString *lineString =
      [[WKBLineString alloc] initWithHasZ:hasZ andHasM:hasM];
...
+ (WKBPolygon *)createPolygonWithHasZ:(BOOL)hasZ andHasM:(BOOL)hasM {
  ...
  for (int i = 0; i < numLineStrings; i++) {
    [polygon
        addRing:[self createLineStringWithHasZ:hasZ andHasM:hasM andRing:true]];
  }
...
+ (NSDecimalNumber *)roundDouble:(double)value withScale:(int)scale {
  NSDecimalNumberHandler *rounder =
